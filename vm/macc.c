...
static value env_access(env e, uint32_t index)
{   
...
static void decode_str(code8 *c8, code *c)
{
	size_t bytes_len = strlen(*c8);
    size_t actual_len = mbstowcs(*c, *c8, bytes_len);

	*c8 += bytes_len + 1;
	*c += actual_len + 1;
}

uint32_t decode_uint32(code8 bytes)
{
	uint32_t value = 0;
	value |= bytes[3] << 24;
	value |= bytes[2] << 16;
...
static code code8_to_code32(code8 c8, size_t s)
{
	code c = GC_malloc(s * sizeof(uint32_t));
	code code_start = c;
	int done = 0;
...
int main(int argc, char **argv)
{
	...
	codeptr = code8_to_code32(codeptr8, sb.st_size);
